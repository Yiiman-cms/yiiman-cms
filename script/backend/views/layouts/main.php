...
$assets=MaterialDashboardProAsset::register($this);
...
?>
<!DOCTYPE html>
<html ng-app="app" lang="<?= Yii::$app->language ?>" layout="<?= Layout::run() ?>">
<head>
    <link rel="manifest" href="<?= $assets->baseUrl ?>/pwa/manifest.json">
...
<div class="loading">
    <style>
        ...
        @keyframes loading {
            from {background-position: 10px 64px;}
            to {background-position: 160px -10px;}
        }
        .loading{
            position: fixed;top:0;right:0;width: 100vw;height: 100vh;z-index: 9999;
            ...
            background-image: url(<?= Yii::$app->Options->pattern ?>);
    </style>
    ...
    <div class="main-panel">
        <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top "
            ...
            <div class="container-fluid">
                ...
                <div class="navbar-wrapper" <?= TippyWidget::attribute(
                    Yii::t('app', 'با این کلید، منوی سمت راست پنل را کوچک و بزرگ کنید')
                ) ?>>
                ...
                <div class="collapse navbar-collapse justify-content-end">
                    <ul class="navbar-nav">
                        <li class="nav-item" <?= TippyWidget::attribute(Yii::t('app', 'داشبورد پنل و آمار ها')) ?>>
                            <a class="nav-link" href="<?= Yii::$app->Options->BackendUrl ?>">
                        ...
                        <li class="nav-item" <?= TippyWidget::attribute(
                            Yii::t('app', 'بخش تنظیمات اصلی کلی سیستم')
                        ) ?>>
                            <a class="nav-link" href="<?= Yii::$app->Options->BackendUrl ?>/setting">
                                <i class="material-icons">settings</i>

                            </a>
                        </li>
                        <li class="nav-item" <?= TippyWidget::attribute(
                            Yii::t('app', 'ورود به صفحه ی اصلی سایت')
                        ) ?>>
                            <a class="nav-link" href="<?= Yii::$app->Options->URL ?>">
                                <i class="material-icons">home</i>
                            </a>
                        </li>
                        <?php Event::trigger(Triggers::className(), Triggers::TOP_BAR_NOTIFICATION_LI) ?>
                        <li <?= TippyWidget::attribute(Yii::t('app', 'خروج از حساب کاربری')) ?> class="nav-item"
                                                                                                data-method="post"
                                                                                                data-confirm="<?= Yii::t(
                                                                                                    'app',
                                                                                                    'are you sure for logout from account?'
                                                                                                ) ?>">
                            <a class="nav-link"
                               href="<?= Yii::$app->urlManager->createUrl('/logout') ?>">
        ...
        ?>
        <div class="panel-background-image" id="back-image-admin"
             style="background-image: url(<?= $image ?>) ">
        </div>
    </div>
</div>
<?php
if (Yii::$app->Options->bubble) {
    ?>
    <div id="large-header" class="large-header" style="height: 1074px;">
        <canvas id="page-blob" width="1600" height="1074"></canvas>
...
<div style="position: fixed;z-index: 999;bottom: 0px;right: 100px">
    Execute At
    <?php
    echo round(microtime(true) - $_SERVER["REQUEST_TIME_FLOAT"], 3);
    ?>
    Sec
</div>
<?php
if (Yii::$app->Options->bubble) {
    ...
    <script>
        ...
        new RainyDay({
            image: $('#back-image-admin')[0] // [Required][Image Id or Image element itself, Id of Element having background Image or Element itself]
        })
